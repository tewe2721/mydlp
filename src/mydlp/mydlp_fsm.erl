...
'REQ_OK'(#state{addr=Addr} = State, Files, Peer) ->
	case mydlp_acl:q(Addr, dest, Files) of
		...
		{quarantine, AclR} -> log_req(State, Peer, quarantine, AclR),
					mydlp_api:quarantine(Files),
					'BLOCK_REQ'(block, State);
		{block, AclR} -> log_req(State, Peer, block, AclR),
					'BLOCK_REQ'(block, State);
		{log, AclR} -> log_req(State, Peer, log, AclR),
					'CONNECT_REMOTE'(connect, State, Peer); % refine this
		{pass, AclR} -> log_req(State, Peer, pass, AclR),
					'CONNECT_REMOTE'(connect, State, Peer)
	end.
